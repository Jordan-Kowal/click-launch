...
func TestGet_AggregatesMultipleRows(t *testing.T) {
	...
	if data.CPU != 7.3 {
		t.Errorf("expected CPU 7.3, got %f", data.CPU)
	}
...
func TestGet_MultipleProcessesParallel(t *testing.T) {
	...
	if data1.CPU != 10.0 || data1.MemoryBytes != 102400*1024 {
		t.Errorf("proc-1: expected CPU=10.0 mem=%d, got CPU=%f mem=%d", 102400*1024, data1.CPU, data1.MemoryBytes)
	}
	data2, ok := result["proc-2"]
	if !ok {
		t.Fatal("expected proc-2 in result")
	}
	if data2.CPU != 3.5 || data2.MemoryBytes != 51200*1024 {
		t.Errorf("proc-2: expected CPU=3.5 mem=%d, got CPU=%f mem=%d", 51200*1024, data2.CPU, data2.MemoryBytes)
	}
